...
class Trois(Model):
    def __init__(self):
        self.possibleNumbers = [1,2,4,8,16]
        self.restart()
    ...
    def process(self):
        ...
        if self.time<0:
            ...
            if ok : self.numberToInsert = self.possibleNumbers[random.randint(0, len(self.possibleNumbers)-1)]
        return True
    ...
    def processKeys(self,keysPressed):
        ...
        if keysPressed[pygame.K_r]:
            ...
            f.close()
            self.restart()
        elif keysPressed[pygame.K_LEFT]:self.rowToInsert = max(0,self.rowToInsert-1)
        elif keysPressed[pygame.K_RIGHT]:self.rowToInsert = min(4,self.rowToInsert+1)
        return True
        ...
        for line in range(0,len(grid)):
            for row in range(0,len(grid[line])):
                if row == self.rowToInsert:
                    blittable.append([number(self.numberToInsert).display(), (initialx+row*rectSize, initialy-(len(grid))*rectSize)])
                if grid[line][row] != None:
                    blittable.append([grid[line][row].display(),(initialx+row*rectSize,initialy+len(grid)-line*rectSize)])
                else:
                    #+str(line)+','+str(row)
                    blittable.append([font.render(".",1,fontcolor),(initialx+row*rectSize,initialy+len(grid)-line*rectSize)])
